...
namespace Vline.Web.DispatchManageSystem.BigDataAnQuan
{
    ...
    /// NewGetYhsj 的摘要说明
    /// </summary>
    public class NewGetYhsj : IHttpHandler
    {
        BLL.DM_BUSI_YHSJ b_YearPlanType = new BLL.DM_BUSI_YHSJ();
        Model.DM_BUSI_YHSJ m_YearPlanType = new Model.DM_BUSI_YHSJ();
        ...
        private string AddType(Dictionary<string, object> dictionary)
        {
            try
            {
                ...
                m_YearPlanType.PCSL =Convert.ToInt32( dictionary["PCSL"].ToString());
                m_YearPlanType.WCSL = Convert.ToInt32(dictionary["WCSL"].ToString());
                m_YearPlanType.WCBL = dictionary["WCBL"].ToString();
                m_YearPlanType.KHFS = dictionary["KHFS"].ToString();
