...
namespace Vline.Web.BigDataAnQuan
{

    /// <summary>
    /// GetYhsl 的摘要说明
    /// </summary>
    public class GetYhsl : IHttpHandler
    {
        Model.DM_BUSI_YHSL m_pcd = new Model.DM_BUSI_YHSL();
        BLL.DM_BUSI_YHSL b_pcd = new BLL.DM_BUSI_YHSL();
        ...
        public void ProcessRequest(HttpContext context)
        {
            ...
            if (updated != null | inserted != null | deleted != null)
            {
                try
                {
                    List<Model.DM_BUSI_YHSL> list_insert = inserted == null ? null : javaScriptSerializer.Deserialize<List<Model.DM_BUSI_YHSL>>(inserted);
                    List<Model.DM_BUSI_YHSL> list_update = updated == null ? null : javaScriptSerializer.Deserialize<List<Model.DM_BUSI_YHSL>>(updated);
                    List<Model.DM_BUSI_YHSL> list_delete = deleted == null ? null : javaScriptSerializer.Deserialize<List<Model.DM_BUSI_YHSL>>(deleted);
        ...
        protected int opreate(List<Model.DM_BUSI_YHSL> insert, List<Model.DM_BUSI_YHSL> update, List<Model.DM_BUSI_YHSL> delete)
        {
            ...
            if (insert != null)
            {
                foreach (Model.DM_BUSI_YHSL m_insert in insert)
                {
                    //添加相应记录
            ...
            if (update != null)
            {
                foreach (Model.DM_BUSI_YHSL m_update in update)
                {
                    //修改的相应记录
            ...
            if (delete != null)
            {
                foreach (Model.DM_BUSI_YHSL m_delete in delete)
                {
                    //删除相应记录
        ...
        public string GetAddString(Vline.Model.DM_BUSI_YHSL model)
        {
            StringBuilder strSql = new StringBuilder();
            strSql.Append("insert into DM_BUSI_YHSL(");
            strSql.Append("YIJI,ERJI,SANJI,SIJI,Updatetime )");

            strSql.Append("values ('" + model.YIJI + "','" + model.ERJI + "','" + model.SANJI + "','" + model.SIJI + "','" + DateTime.Now + "') ");
        ...
        /// 获得修改一条数据的sql
        /// </summary>
        public string GetUpdateString(Vline.Model.DM_BUSI_YHSL model)
        {
            StringBuilder strSql = new StringBuilder();
            strSql.Append("update DM_BUSI_YHSL");
            strSql.Append(" set YIJI='" + model.YIJI + "',ERJI='" + model.ERJI + "',SANJI='" + model.SANJI + "',SIJI='" + model.SIJI + "',Updatetime='" + DateTime.Now + "' where Id=" + model.Id + " ");
            return strSql.ToString();
        }
        ...
        {
            return "delete DM_BUSI_YHSL where Id=" + P_id + "";
        }
