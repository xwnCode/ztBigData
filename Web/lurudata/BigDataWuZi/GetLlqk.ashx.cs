...
namespace Vline.Web.DispatchManageSystem.BigDataWuZi
{
    ...
    /// GetLlqk 的摘要说明
    /// </summary>
    public class GetLlqk : IHttpHandler
    {
        BLL.DM_BUSI_LLQK b_YearPlanType = new BLL.DM_BUSI_LLQK();
        Model.DM_BUSI_LLQK m_YearPlanType = new Model.DM_BUSI_LLQK();
        ...
        private string AddType(Dictionary<string, object> dictionary)
        {
            try
            {
                ...
                m_YearPlanType.JCW = Convert.ToInt32(dictionary["JCW"].ToString());
                m_YearPlanType.DL = Convert.ToInt32(dictionary["DL"].ToString());
                m_YearPlanType.BD = Convert.ToInt32(dictionary["BD"].ToString());
                m_YearPlanType.TX = Convert.ToInt32(dictionary["TX"].ToString());

                m_YearPlanType.XX = Convert.ToInt32(dictionary["XX"].ToString());
                m_YearPlanType.XH = Convert.ToInt32(dictionary["XH"].ToString());
                m_YearPlanType.Fas = Convert.ToInt32(dictionary["Fas"].ToString());
                m_YearPlanType.Bas = Convert.ToInt32(dictionary["Bas"].ToString());
