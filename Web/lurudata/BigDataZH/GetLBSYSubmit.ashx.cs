...
namespace Vline.Web.lurudata.BigDataZH
{
    ...
    /// GetLBSYSubmit 的摘要说明
    /// </summary>
    public class GetLBSYSubmit : IHttpHandler
    {
        ...
        public string GetData()
        {
            try
            {
                DataSet dsMessage = b_YearPlanType.GetAllList("Worktype='拉拔试验'");
        ...
        private string AddType(Dictionary<string, object> dictionary)
        {
            try
            {
                ...
                m_YearPlanType.Lbsyzh = float.Parse(dictionary["Lbsyzh"].ToString());
                m_YearPlanType.Pausetime = float.Parse(dictionary["Pausetime"].ToString());
